...
(function () {
  ...
  /* "The hero" for this purpose is the video stage AND the title block, not the
     video alone: the rail may only appear once the title, authors and links are
     completely off screen. Watching the stage alone brought it in while the
     title was still being read, which is exactly the moment a fixed index is
     noise. So both elements are observed and the flag is their OR -- true while
     either is even one pixel on screen. */
  function initRail() {
